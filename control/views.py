...
class ControlList(TemplateView):
    ...
    def post(self, request):
        ...
        if filter_form.is_valid():
            ...
            group_status = group_status[group_query_idx]

        if group_query_idx:
            filtered_groups = Group.objects.filter_by_status(group_status)

        queryset_list = Control.objects.filter_by_group_status(group_status) \
            .filter(published__range=[period_initial, period_final])
